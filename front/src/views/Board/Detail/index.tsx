...
export default function BoardDetail() {
     ...
     const BoardBotttom = () => {
          ...
          const { totalPage, currentPage, currentSection,
                  onNextClickHandler, onPreviousClickHandler, onPageClickHandler, changeSection } = usePagination();
          ...
          const onShowFavoriteButtonClickHanlder = () => {
          ...
          //                            effect                            //
          // description : 조회하는 boardNumber 가 변경될때 마다 실행될 함수 //
          useEffect(()=>{
               if(!boardNumber) {
                    alert('잘못된 접근입니다.');
                    navigator(MAIN_PATH);
          ...
          // description : favorite list 가 변경될때 마다 실행될 함수 //
          useEffect(() => {
               setFavorite(false);
               if(!user) return;
               favoriteList.forEach(item => {if(item.email === user.email) setFavorite(true)})
          }, [favoriteList]);
          // description : current page 가 변경될때 마다 실행될 함수 //
          useEffect(() => {
               getViewCommentList(commentList);
          }, [currentPage]);
          // description : current section 이 변경될때 마다 실행될 함수 //
          useEffect(() => {
               ...
               <div className='board-bottom'>
                    <div className='board-bottom-button-container'>
                         <div className='board-bottom-button-group'>
                              ...
                              <div className='board-detail-bottom-button' onClick={onShowFavoriteButtonClickHanlder}>
                                   {showFavorite ? (
                    ...
                    <div className='board-comments-container'>
                         ...
                         <div className='board-comments-input-box'>
                              <div className='board-comments-input-container'>
                                   <textarea className='board-comments-input' placeholder='댓글을 작성해주세요.' value={comment} onChange={onCommentChangeHandler}></textarea>
                                   <div className='board-comments-button-box'>
                                        {comment.length === 0 ? (
                                        <div className='disable-button'>댓글달기</div>
                                        ) : (
                                        <div className='black-button' onClick={onPostCommentClickHandler}>댓글달기</div>
                                        )}
                                   </div>
                              </div>
                         </div>
